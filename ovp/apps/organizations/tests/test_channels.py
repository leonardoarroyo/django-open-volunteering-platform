...
class OrganizationChannelTestCase(TestCase):
  def setUp(self):
    self.channel = Channel.objects.create(name="Test channel", slug="test-channel")
    self.user1 = User.objects.create_user(email="test@email.com", password="testpwpw", object_channel="default")
    self.user2 = User.objects.create_user(email="test@email.com", password="testpwpw", object_channel="test-channel")
  ...
  def test_organizations_are_created_on_correct_channel(self):
    ...
    client = APIClient()
    client.login(email="test@email.com", password="testpwpw", channel="test-channel")

    response = client.post(reverse("organization-list"), data, format="json")
    self.assertTrue(response.status_code == 400)

    response = client.post(reverse("organization-list"), data, format="json", HTTP_X_OVP_CHANNEL="test-channel")
  ...
  def test_cant_modify_organizations_on_another_channel(self):
    ...
    client.login(email="test@email.com", password="testpwpw", channel="default")
    response = client.patch(reverse("organization-detail", ["test-organization"]), {"description": "another description"}, format="json", HTTP_X_OVP_CHANNEL="test-channel")
    self.assertTrue(response.status_code == 400)

    client.login(email="test@email.com", password="testpwpw", channel="default")
    response = client.patch(reverse("organization-detail", ["test-organization"]), {"description": "another description"}, format="json", HTTP_X_OVP_CHANNEL="default")
