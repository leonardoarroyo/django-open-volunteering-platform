...
class OrganizationBookmarkTestCase(TestCase):
  def setUp(self):
    ...
    self.organization = Organization.objects.create(name="test organization", description="test organization", owner=self.user, object_channel="default")
  ...
  def test_can_bookmark(self):
    ...
    response = self.client.post(reverse("organization-bookmark", ["test-organization"]), format="json")
    ...
    self.assertEqual(OrganizationBookmark.objects.first().organization, self.organization)
  ...
  def test_can_unbookmark(self):
    ...
    self.test_can_bookmark()
    self.assertEqual(OrganizationBookmark.objects.count(), 1)
    response = self.client.post(reverse("organization-unbookmark", ["test-organization"]), format="json")
    self.assertEqual(response.status_code, 200)
  ...
  def test_can_retrive_bookmarked(self):
    ...
    response = self.client.get(reverse("organization-bookmarked"), format="json")
  ...
  def test_cant_access_bookmark_routes_logged_out(self):
    ...
    response = client.post(reverse("organization-bookmark", ["test-organization"]), format="json")
    self.assertTrue(response.status_code == 401)

    response = client.post(reverse("organization-unbookmark", ["test-organization"]), format="json")
    self.assertTrue(response.status_code == 401)
