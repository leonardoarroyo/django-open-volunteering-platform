...
@ChannelViewSet
class GalleryResourceViewSet(mixins.CreateModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
  lookup_field = "uuid"
  ...
  def get_permissions(self):
    ...
    if self.action == 'partial_update':
      self.permission_classes = (permissions.IsAuthenticated, GalleryEditAllowed)
    if self.action == 'retrieve':
      self.permission_classes = ()
    if self.action == 'delete':
      self.permission_classes = (permissions.IsAuthenticated, GalleryEditAllowed)
    return super(GalleryResourceViewSet, self).get_permissions()
