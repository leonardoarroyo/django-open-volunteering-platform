...
class GalleryEditAllowed(permissions.BasePermission):
  """ Permission that allows an gallery owner to edit the gallery. Members of organization can edit the gallery if it is associated with a project or project.post. """
  ...
  def has_object_permission(self, request, view, obj):
    if request.user.is_authenticated:
      ...
      projects = list(Project.objects.filter(Q(galleries=obj) | Q(posts__gallery=obj)))
      if len(projects):
