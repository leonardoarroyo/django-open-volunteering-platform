...
from ovp.apps.search.querysets import get_project_queryset, get_organization_queryset
...
def fetch_user_relevance_catalogue(catalogue, queryset, request, context=None, serializer=None):
  ...
  queryset = filters.UserSkillsCausesFilter().\
              annotate_queryset(queryset, request.user).\
...
def fetch_catalogue(catalogue_dict, serializer=False, request=None, context=None, channel='default', slug=None):
  """
  ...
  organization_base_queryset = get_organization_queryset(request=request).filter(pk__in=result_keys)
  ...
  for i, section in enumerate(catalogue_dict["sections"]):
    ...
    if serializer:
      if catalogue_dict["sections"][i]["type"] == "organizations":
        result = OrganizationSearchSerializer(qs, many=True, context=context).data
      else:
        result = ProjectSearchSerializer(qs, many=True, context=context).data
    else:
      result = qs

    catalogue_dict["sections"][i][catalogue_dict["sections"][i]["type"]] = result
