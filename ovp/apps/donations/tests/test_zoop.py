...
class TestZoopBackend(TestCase):
  ...
  def test_attach_customer_to_token(self):
    ...
    token = self.backend.generate_card_token(holder_name="Test", expiration_month="03", expiration_year="2018", security_code="123", card_number="5201561050024014").json()["id"]
    customer = self.backend.create_customer(first_name="Abraham", last_name="Lincoln", description="Third sector donator", email="abrahamlincoln@usa.gov").json()["id"]
    response = self.backend.attach_token_to_customer(token=token, customer=customer)
  ...
  def test_create_customer(self):
    response = self.backend.create_customer(first_name="Abraham", last_name="Lincoln", description="Third sector donator", email="abrahamlincoln@usa.gov")
    self.assertEqual(response.status_code, 201)
    self.assertTrue(response.json()["id"])