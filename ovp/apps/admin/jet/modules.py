...
class Indicators(DashboardModule):
    ...
    def init_with_context(self, context):
      ...
      applies_distinct = self.date_filter(apply_distinct_qs, date_min, date_max, "date")

      self.organizations_count = organizations_created.count()
      self.organizations_published_count = organizations_published.filter(published=True).count()
      self.projects_count = projects_created.count()
      self.projects_published_count = projects_published.filter(published=True).count()
