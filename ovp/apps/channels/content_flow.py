...
class NoContentFlow(Exception):
  """ Raised by a content flow class to indicate there should not be a content flow for a given model  """
  pass
...
class ContentFlowManager():
  ...
  def filter_searchqueryset(self, destination_channel, queryset):
    if len(queryset.query.models) != 1:
      raise Exception("Channel content flow doesn't implement a solution for multiple models SearchQuerySet")
  ...
  def _filter_qs_or_sqs(self, destination_channel, Q_SQ, channel_filter_name, model_class, qs):
    ...
    if type(self.flow_dict.get(destination_channel, None)) == list:
      for flow_item in self.flow_dict[destination_channel]:
        ...
        content_flow_q_obj.add(Q_SQ(**{channel_filter_name: flow_item.source}), Q_SQ.AND)

        try:
          content_flow_q_obj.add(getattr(flow_item, filter_method)(model_class), Q_SQ.AND)
