...
class ProjectChannelTestCase(TestCase):
  def setUp(self):
    ...
    user = User.objects.create_user(email=self.email, password=self.password, object_channel="default")
    user = User.objects.create_user(email=self.email, password=self.password, object_channel="test-channel")
  ...
  def test_applies_are_saved_on_correct_channel(self):
    ...
    response = client.post(reverse("project-applies-apply", ["test-project"]), format="json", HTTP_X_OVP_CHANNEL="test-channel")
  ...
  def test_cant_apply__from_another_channel(self):
    ...
    response = client.post(reverse("project-applies-apply", ["test-project"]), format="json", HTTP_X_OVP_CHANNEL="default")
    self.assertTrue(response.status_code == 400)
