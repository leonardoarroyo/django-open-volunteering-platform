...
class RatingViewsTest(TestCase):
  def setUp(self):
    self.user = User.objects.create_user(name="a", email="testmail@test.com", password="test_returned", object_channel="default")
    self.user2 = User.objects.create_user(name="b", email="testmail2@test.com", password="test_returned", object_channel="default")
    ...
    rp1 = RatingParameter.objects.create(slug="project-score", type=2, object_channel="default")
    ...
    self.client.login(username="testmail@test.com", password="test_returned", channel="default")
  ...
  def test_retrieve_rating_request_parameters(self):
    ...
    self.assertEqual(response.data[0]["rating_parameters"][0]["slug"], "user-has-shown")
    self.assertEqual(response.data[0]["rating_parameters"][0]["type"], "Boolean")

    self.assertEqual(len(response.data[2]["rating_parameters"]), 2)
    self.assertEqual(response.data[2]["rating_parameters"][0]["slug"], "project-score")
    self.assertEqual(response.data[2]["rating_parameters"][0]["type"], "Quantitative")
    self.assertEqual(response.data[2]["rating_parameters"][1]["slug"], "project-how-was-it")
    self.assertEqual(response.data[2]["rating_parameters"][1]["type"], "Qualitative")

  def test_cant_rate_if_not_requested(self):
    uuid = str(RatingRequest.objects.first().uuid)
    self.client.force_authenticate(self.user2)
    response = self.client.post(reverse("rating-request-rate", [uuid]), {}, format="json")
  ...
  def test_rate_validation(self):
    ...
    self.assertEqual(response.json()["non_field_errors"][0], "Invalid parameter 'project-dont-exist' for request.")
    ...
    response = self.client.post(reverse("rating-request-rate", [uuid]), data, format="json")
    self.assertEqual(response.status_code, 400)
    self.assertEqual(response.json()["non_field_errors"][0], "You have sent multiple answers for a parameter. Check you request body.")
    ...
    response = self.client.post(reverse("rating-request-rate", [uuid]), data, format="json")
    self.assertEqual(response.status_code, 400)
    self.assertEqual(response.json()["non_field_errors"][0], "The following parameters are missing: project-score, project-how-was-it.")
  ...
  def test_can_rate(self):
    ...
    self.assertEqual(Rating.objects.first().request, RatingRequest.objects.last())
    self.assertEqual(Rating.objects.first().answers.count(), 2)

    self.assertEqual(Rating.objects.first().answers.first().parameter.slug, "project-score")
    self.assertEqual(Rating.objects.first().answers.first().value_quantitative, 1)
  ...
  def test_quantitative(self):
    ...
    response = self.client.post(reverse("rating-request-rate", [uuid]), data, format="json")
    self.assertEqual(response.status_code, 400)
    self.assertEqual(response.data['answers'][0]['non_field_errors'][0], "'value_quantitative' must be between 0 and 1 for qualitative parameters.")
  ...
  def test_boolean(self):
    ...
    response = self.client.post(reverse("rating-request-rate", [uuid]), data, format="json")
    self.assertEqual(response.status_code, 400)
    self.assertEqual(response.data['answers'][0]['non_field_errors'][0], "'value_quantitative' must be 0 or 1 for boolean parameters.")
