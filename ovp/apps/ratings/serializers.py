...
class RatedObject(RelatedField):
  def to_representation(self, obj):
    ...
    if obj_type == 'user':
      return ShortUserPublicRetrieveSerializer(obj, context=self.context).data
    elif obj_type == 'project':
      return ProjectRetrieveSerializer(obj, context=self.context).data
    elif obj_type == 'organization':
      return OrganizationRetrieveSerializer(obj, context=self.context).data
...
class RatingRequestRetrieveSerializer(ChannelRelationshipSerializer):
  ...
  rating_parameters = RatingParameterRetriveSetrializer(many=True, read_only=True)

  class Meta:
    model = RatingRequest
    fields = ['uuid', 'created_date', 'rated_object', 'object_type', 'rating_parameters']
...
class RatingAnswerCreateSerializer(Serializer):
  ...
  class Meta:
    fields = ['rating', 'parameter_slug', 'value_qualitative', 'value_quantitative']

  def create(self, data, *args, **kwargs):
    # Parameter
    data['parameter'] = RatingParameter.objects.get(slug=data.pop('parameter_slug', None))

    # Rating
    data['rating'] = self.context['rating']

    return RatingAnswer.objects.create(object_channel=self.context['request'].channel, **data)

  def validate(self, data, *args, **kwargs):
    try:
      parameter = RatingParameter.objects.get(slug=data['parameter_slug'])
    except RatingParameter.DoesNotExist:
      return super(RatingAnswerCreateSerializer, self).validate(data, *args, **kwargs)
    ...
    if parameter.type == 2:
      ...
      if value is not None:
        if value < 0 or value > 1:
          raise ValidationError('\'value_quantitative\' must be between 0 and 1 for qualitative parameters.')
    ...
    if parameter.type == 3:
      ...
      if value is not None:
        if value != 0 and value != 1:
          raise ValidationError('\'value_quantitative\' must be 0 or 1 for boolean parameters.')
    return super(RatingAnswerCreateSerializer, self).validate(data, *args, **kwargs)
...
class RatingCreateSerializer(ChannelRelationshipSerializer):
  ...
  def create(self, data, *args, **kwargs):
    ...
    rating = super(RatingCreateSerializer, self).create(data, *args, **kwargs)

    self.context['rating'] = rating
    for answer in answers:
      serializer = RatingAnswerCreateSerializer(data=answer, context=self.context)
  ...
  def validate(self, data, *args, **kwargs):
    ...
    tmp_rating_request_parameter_slugs = [x.slug for x in rating_request.rating_parameters.all()]
    ...
    self.validate_missing_parameters(data, tmp_rating_request_parameter_slugs)
    self.validate_extra_parameters(data, tmp_rating_request_parameter_slugs)

    return super(RatingCreateSerializer, self).validate(data, *args, **kwargs)

  def validate_duplicated_parameters(self, data):
    slugs = [answer["parameter_slug"] for answer in data["answers"]]

    if len(slugs) != len(set(slugs)):
      raise ValidationError("You have sent multiple answers for a parameter. Check you request body.")
  ...
  def validate_missing_parameters(self, data, request_parameter_slugs):
    ...
    if len(missing):
      raise ValidationError("The following parameters are missing: {}.".format(", ".join(missing)))
