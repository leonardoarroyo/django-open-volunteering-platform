...
class ChannelsTestCase(TestCase):
  def setUp(self):
    TestChannel = Channel.objects.create(name="Test Channel", slug="test-channel")
  ...
  def test_startup_route(self):
    ...
    client = APIClient()
    response = client.get(reverse("startup"), format="json", HTTP_X_OVP_CHANNEL="default")
    self.assertEqual(len(response.data["skills"]), 12)
    self.assertEqual(len(response.data["causes"]), 13)

    response = client.get(reverse("startup"), format="json", HTTP_X_OVP_CHANNEL="test-channel")
  ...
  def test_contact_route(self):
    ...
    data = {"name": "my-name", "message": "my message", "email": "reply_to@asddsa.com", "phone": "+5511912345678", "recipients": ["test-channel-contact@1.com"]}
    ChannelContact.objects.create(email="test-channel-contact@1.com", object_channel="test-channel")

    response = self.client.post(reverse("contact"), data=data, format="json", HTTP_X_OVP_CHANNEL="default")
    self.assertTrue(response.status_code == 400)

    response = self.client.post(reverse("contact"), data=data, format="json", HTTP_X_OVP_CHANNEL="test-channel")
  ...
  def test_leads_route(self):
    ...
    self.assertEqual(Lead.objects.filter(channel__slug="test-channel").count(), 0)
