...
class GenericUserTokenViewSet(viewsets.GenericViewSet):
  """
  Generates generic tokens for users. Used by password recovery and email verification.
  """
  ...
  def create(self, request, *args, **kwargs):
    ...
    if request.user.is_authenticated:
      ...
      try:
        ...
        user = self.get_queryset().filter(channel__slug=request.channel).get(email__iexact=email)
      except:
        user = None
    ...
    if user:
      ...
      tokens = self.Token.objects.filter(user=user, created_date__gte=to_check, channel__slug=request.channel)
      ...
      token = self.Token.objects.create(user=user, object_channel=request.channel)
